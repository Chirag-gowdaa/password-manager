...
const Manager = () => {
  ...
  const fetchPasswords = async () => {
    const res = await fetch(`${import.meta.env.VITE_API_BASE}/api/passwords`);
  ...
  const savePassword = async () => {
    ...
    const res = await fetch(`${import.meta.env.VITE_API_BASE}/api/save`, {
      method: "POST",
          ...
          passwords.map((item) => (
            <div key={item._id} className="text-sm border-b border-[#2D333B] py-2 bg-gray-800 p-3 rounded">
              ...
              <div className="flex justify-between items-center">
                <p className="text-white"><strong>Password:</strong> {item.password}</p>
                <button onClick={() => handleDelete(item._id)} className="text-red-500 hover:text-red-700">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                    <path fillRule="evenodd" d="M6 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm4 0a1 1 0 012 0v6a1 1 0 11-2 0V8zm2-4a2 2 0 00-2-2H8a2 2 0 00-2 2v1H4a1 1 0 000 2h1v9a2 2 0 002 2h6a2 2 0 002-2V7h1a1 1 0 100-2h-2V4zM8 4h4v1H8V4z" clipRule="evenodd" />
                  </svg>
                </button>
              </div>
            </div>
